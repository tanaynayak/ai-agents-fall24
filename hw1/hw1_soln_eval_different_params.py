...
def generate_predictions(batch, num_beams=4, top_k=50, top_p=1.0, temperature=1.0):
...
def evaluate_with_parameters(num_samples=None, num_beams=4, top_k=50, top_p=1.0, temperature=1.0):
    """
    Evaluate the model using batch processing with configurable generation parameters for a limited number of samples.
    ...
    for batch in tqdm(validation_loader):
        ...
        preds = generate_predictions(batch, num_beams=num_beams, top_k=top_k, top_p=top_p, temperature=temperature)
...
configs = [
    {"num_beams": 4, "top_k": 50, "top_p": 1.0, "temperature": 1.0},  # Baseline
    {"num_beams": 6, "top_k": 50, "top_p": 1.0, "temperature": 1.0},  # Higher beam size
    {"num_beams": 4, "top_k": 20, "top_p": 1.0, "temperature": 1.0},  # Lower top-k
    {"num_beams": 4, "top_k": 50, "top_p": 0.9, "temperature": 1.0},  # Lower top-p (nucleus sampling)
    {"num_beams": 4, "top_k": 50, "top_p": 1.0, "temperature": 0.7},  # Lower temperature
]
...
for config in configs:
    print(f"Evaluating with config: {config} on {num_samples_to_evaluate} samples...")
